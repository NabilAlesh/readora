...
class UserCubit extends Cubit<UserStates> {
  ...
  Future<void> userLogin() async {
    ...
    try {
      ...
      if (error.type == DioExceptionType.badResponse) {
        emit(
          AuthError(error: error.response?.data['message'] ?? 'Login failed'),
        );
      } else {
  ...
  Future<void> userRegister() async {
    ...
    try {
      ...
      await Network.postData(url: Urls.userRegister, data: formData);
  ...
  Future<void> forgetPassword() async {
    ...
    try {
      await Network.postData(
        ...
        data: {'email': forgetEmailController.text},
      );
      ...
      if (error.type == DioExceptionType.badResponse) {
        emit(
          ForgetPasswordError(
            error: error.response?.data['message'] ?? 'حدث خطأ ما',
          ),
        );
      } else {
  ...
  Future<void> verifyResetOtp() async {
    ...
    try {
      ...
      if (error.type == DioExceptionType.badResponse) {
        emit(
          VerifyOTPError(
            error: error.response?.data['message'] ?? 'الرمز غير صحيح',
          ),
        );
      } else {
